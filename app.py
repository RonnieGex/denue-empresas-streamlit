...
def get_recommendations_from_ai(api_key, provider, prompt):
    try:
        ...
        return f"❌ Error al conectar con el proveedor: {e}"
...
if archivo:
    ...
    if not all(col in df.columns for col in columnas_clave):
        ...
        with st.form("formulario_filtros"):
            ...
            show_ia = st.form_submit_button("🔍 Obtener giros sugeridos")
            if show_ia and tipo_negocio and api_key:
            ...
            ejecutar = st.form_submit_button("📊 Mostrar resultados")
