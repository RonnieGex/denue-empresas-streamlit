...
st.title("🔥 Katalis Ads DB Optimizer")
        ...
        if not all(col in df.columns for col in columnas_clave):
            ...
            with st.form("filtros_form"):
                ...
                if tipo_negocio and api_key:
                    if st.button("🔍 Consultar recomendaciones IA"):
                        prompt = f"Dame 5 giros económicos ideales para un negocio que se dedica a: {tipo_negocio}."
                        with st.spinner("Consultando IA..."):
                            resultado = get_recommendations_from_ai(api_key, proveedor, prompt)
                        st.markdown("🎯 **Sugerencias de giros:**")
                        st.write(resultado.splitlines() if isinstance(resultado, str) else resultado)
