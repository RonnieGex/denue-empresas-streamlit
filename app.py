...
st.set_page_config(page_title="Katalis Ads DB Optimizer Ultra Pro", layout="wide")
st.markdown("# 🚀 Katalis Ads DB Optimizer Ultra Pro")
st.markdown("Optimiza tu base DENUE con filtros congelados que solo procesan al presionar el botón. Cero recalculo innecesario, máxima velocidad.")
            ...
            with st.expander("🎛️ Filtros avanzados", expanded=True):
                ...
                if not empleados.empty:
                    st.session_state.filtros['rango_emp'] = st.slider(
                        ...
                        int(empleados.min()), int(empleados.max()),
                        (int(empleados.min()), int(empleados.max()))
                    )
