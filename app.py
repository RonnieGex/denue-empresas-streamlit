...
def mostrar_metricas(df_filtrado):
    ...
    if "per_ocu_estimado" in df_filtrado.columns and not df_filtrado["per_ocu_estimado"].isna().all():
...
st.markdown("Optimiza tu base de datos con filtros avanzados. Ahora con métricas y recomendaciones inteligentes.")
    ...
    try:
        ...
        if not all(col in df.columns for col in columnas_clave):
            ...
            if submitted:
                ...
                st.download_button("📥 Descargar CSV optimizado", csv, "denue_filtrado.csv", "text/csv")
