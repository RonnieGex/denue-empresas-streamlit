...
if archivo:
    try:
        ...
        if columnas_faltantes:
            ...
            df_estado = df_estado[df_estado['per_ocu'].notna()]
            if not df_estado.empty:
                per_ocu_validos = df_estado['per_ocu'].astype(int)
                if not per_ocu_validos.empty:
                    min_emp = per_ocu_validos.min()
                    max_emp = per_ocu_validos.max()
                    ...
                    st.warning("⚠️ No hay datos válidos de empleados para generar el filtro.")
            else:
